...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let ca = rcgen::Certificate::from_params({
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, "ca.massive-linx");
        params.name_constraints = Some(NameConstraints {
            permitted_subtrees: vec![GeneralSubtree::DnsName("massive-linx".to_owned())],
    ...
    let server_cert = rcgen::Certificate::from_params({
        let name = "server.massive-linx".to_owned();
    ...
    let client_cert = rcgen::Certificate::from_params({
        let name = "client.massive-linx".to_owned();
        let mut params = rcgen::CertificateParams::new(vec![name.clone()]);
        params.distinguished_name.push(DnType::CommonName, name);
        params.use_authority_key_identifier_extension = true;
        // params.key_usages.push(KeyUsagePurpose::DigitalSignature);
